Class TagParser
    ...
    Private Sub Class_Initialize
        Set ResColl = Server.CreateObject("Scripting.Dictionary")
	...
	Private Function Tag_Parser()
		...
		regex.Pattern = "<cms:function>(\w+?)\((.*?)\)</cms:function>"
		Set matches2 = regex.Execute(TempContent)
		For Each match2 In matches2
			retVal = GetCacheValue(match2.Value)
			If retVal = "" Then 	
				Dim tempArray, strA
				tempArray = Split(match2.SubMatches(1), ",")
				strA = "temp = " & match2.SubMatches(0) & "("
		...
		For Each match In matches
			...
			If retVal = "" Then 			
				If match.SubMatches(0) <> "" And match.SubMatches(1) <> "" And match.SubMatches(2) <> "" Then
					...
					Dim matches5, match5
					regex.Pattern = "\{RECORD_NUM\}"
					set matches5 = regex.Execute(strTemplate)	
					...
					While Not rs.Eof
						...
						For Each match5 In matches5							
							strTemplate2 = Replace(strTemplate2, match5.Value, RECORD_NUM)	
						Next
						...
						For Each match3 In matches3
							If Len(rs(match3.SubMatches(0))) > 0 Then
								strTemplate2 = Replace(strTemplate2, match3.Value, Left(rs(match3.SubMatches(0)), match3.SubMatches(1)))	
							Else
								strTemplate2 = Replace(strTemplate2, match3.Value, "")	
							End If
						Next
										
						For Each match2 In matches2				
								If Len(rs(match2.SubMatches(0))) > 0 Then
									strTemplate2 = Replace(strTemplate2, match2.Value, rs(match2.SubMatches(0)))		
								Else
									strTemplate2 = Replace(strTemplate2, match2.Value, "")	
								End IF
						Next				

						strHTML = strHTML & strTemplate2 
						rs.MoveNext()
					Wend
					rs.Close
					Set rs = Nothing
